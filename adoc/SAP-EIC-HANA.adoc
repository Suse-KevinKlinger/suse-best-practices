...
This chapter descibes a basic example of how to use {hana} with {eic}.
It will *NOT* instruct how to set up {hana} or high availability for {hana}.

For more information around the later one, please refer to SUSE's best practices guides:

link:https://documentation.suse.com/sbp/high-availability/[]

== Architecture overview

IMPORTANT: HANA is supposed to run in a side-by-side approach with the {eic} Kubernetes cluster. {hana} is not part of that Kubernetes cluster.
...
Similar to the previous log in, the our example uses now the newly created _EICDB_ and the password from the commmand above:
...
The _HANA DB Name_ is the name of the tenant datbase to use. +
