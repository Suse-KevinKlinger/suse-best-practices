...
|{pg} | {pg_version}
|{redis} | {redis_version}
|{istio} | {istio_chart_version}
|{kiali} | {kiali_chart_version}
...
// The Restricted Access feature requires the configuration of an {istio} Service Mesh, and Monitoring Tools which will be provided by {kiali}, {prometheus} and {grafana}.
// After installing these applications, you will configure the restricted access for the {elm} so {eic} can be deployed on the cluster.
